...
def create_hashrate_bar_chart(hashrate_data):
    ...
    for i, (val, val_high) in enumerate(zip(values_mid, values_high)):
        label_x = val_high + 2
        ax2.text(label_x, i - 0.55, f'{val:.1f}%', va='bottom', color='white', fontsize=10, fontweight='bold')
...
def create_government_holdings_chart(government_data):
    ...
    # Add value labels - position above error bars (offset up significantly)
    for i, (d, s, s_high) in enumerate(zip(disclosed_btc, secret_btc_mid, secret_btc_high)):
        total = d + s
        max_extent = d + s_high + 10000
        if total > 0:
            ax.text(max_extent, i - 0.55, f'{total:,.0f}', va='bottom', color='white', fontsize=10, fontweight='bold')
