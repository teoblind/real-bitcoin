...
def create_hashrate_bar_chart(hashrate_data):
    ...
    ax2.errorbar(values_mid, y_pos, xerr=errors, fmt='none', color='black', capsize=3)
...
def create_government_holdings_chart(government_data):
    ...
    ax.errorbar(secret_centers, y_pos, xerr=errors, fmt='none', color='black', capsize=3)
    ...
    for i, (d, s) in enumerate(zip(disclosed_btc, secret_btc_mid)):
        total = d + s
        if total > 0:
            ax.text(total + 5000, i, f'{total:,.0f}', va='center', fontsize=9)
