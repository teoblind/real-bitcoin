...
def create_hashrate_bar_chart(hashrate_data):
    ...
    for i, (val, val_high) in enumerate(zip(values_mid, values_high)):
        ...
        ax2.text(label_x, i - 0.3, f'{val:.1f}%', va='center', fontsize=10, fontweight='bold')
...
def create_government_holdings_chart(government_data):
    ...
    # Calculate max extent including error bars for label positioning (offset up by 0.3)
    for i, (d, s, s_high) in enumerate(zip(disclosed_btc, secret_btc_mid, secret_btc_high)):
        ...
        if total > 0:
            ax.text(max_extent, i - 0.3, f'{total:,.0f}', va='center', fontsize=10, fontweight='bold')
